...
async function nextNumber(client) {
  const { rows: nsRows } = await client.query(
    `SELECT prefix, next_number, padding FROM number_series WHERE name='Sales Orders' FOR UPDATE`
  );
  if (!nsRows.length) throw new Error('Sales Order number series not configured');
