...
async function nextNumber(client) {
  const series = await getOrCreateSeries(client, 'Purchase Settlements', 'PS-', 6);
  return safeNextNumber(client, series, 'name', 'Purchase Settlements', 'purchase_settlements', 'number');
}
