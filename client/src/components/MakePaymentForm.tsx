...
export default function MakePaymentForm({ payment, onClose, onSaved }: Props) {
  ...
  async function handleSave(e: React.FormEvent, continueEdit = false) {
    ...
    if (!validInst.length) { setError('At least one payment line with amount is required'); return; }
    setSaving(true);
    try {
      const payload = { vendor_id: Number(vendorId), date, reference: reference || null, notes: notes || null, instruments: validInst };
