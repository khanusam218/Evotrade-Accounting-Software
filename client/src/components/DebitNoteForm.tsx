...
export default function DebitNoteForm({ debitNote, onClose, onSaved }: Props) {
  const isEdit = debitNote !== null;
  const status = isEdit ? debitNote.status : 'draft';

  const [number, setNumber]         = useState(isEdit ? debitNote.number : '');
  const [date, setDate]             = useState(isEdit ? debitNote.date.slice(0, 10) : today());
  const [contactName, setContact]   = useState(isEdit ? debitNote.contact_name : '');
  const [reference, setReference]   = useState(isEdit ? (debitNote.reference ?? '') : '');
  const [accountId, setAccountId]   = useState(isEdit ? String(debitNote.account_id) : '');
  const [amount, setAmount]         = useState(isEdit ? String(debitNote.amount) : '');
  const [comments, setComments]     = useState(isEdit ? (debitNote.comments ?? '') : '');
  const [autoSettle, setAutoSettle] = useState(isEdit ? debitNote.auto_settle : true);
  ...
  async function handleSave(addNew = false) {
    ...
    setSaving(true);
    try {
      if (isEdit) await updateDebitNote(debitNote.id, payload);
    ...
    <div className="w-full bg-white flex flex-col">
        ...
        <div className="flex items-center justify-between border-b border-gray-200 px-6 py-4">
          <h2 className="text-lg font-bold text-gray-900">
            Debit Notes - [{isEdit ? debitNote.number : (numberLoading ? '…' : number)}]
          </h2>
          ...
          <div className="grid grid-cols-4 gap-4">
            ...
            <div>
              ...
              <div className="flex gap-1">
                ...
                {!isEdit && (
                  <button type="button"
                    onClick={() => { setNumberLoading(true); getNextDNNumber().then(setNumber).finally(() => setNumberLoading(false)); }}
