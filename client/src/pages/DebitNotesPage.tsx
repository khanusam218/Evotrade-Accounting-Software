...
import {
  approveDebitNote, cancelDebitNote, deleteDebitNote, getDebitNotes,
} from '../api/debitNotes';
import type { DebitNote } from '../types/debitNote';
import { DN_STATUS_COLORS, DN_STATUS_LABELS } from '../types/debitNote';
...
function printDebitNotes(rows: DebitNote[]) {
  ...
  win.document.write(`<!DOCTYPE html><html><head><meta charset="utf-8"/><title>Debit Notes</title>
...
<div class="hdr"><span class="co">Evotrade</span><span class="ti">Debit Notes</span></div><hr/>
...
function exportDebitNotesToExcel(rows: DebitNote[]) {
  ...
  const a=document.createElement('a');a.href=url;a.download='DebitNotes.csv';
...
export default function DebitNotesPage() {
  ...
  const [editEntry, setEditEntry] = useState<DebitNote | null>(null);
  ...
  const fetchNotes = useCallback(async () => {
    ...
    try {
      ...
      setError(`Failed to load debit notes: ${err instanceof Error ? err.message : 'Unknown error'}`);
  ...
  const visibleNotes = notes.filter((dn) => {
    if (appliedFilters.contact   && !dn.contact_name.toLowerCase().includes(appliedFilters.contact.toLowerCase())) return false;
    if (appliedFilters.reference && !String(dn.reference ?? '').toLowerCase().includes(appliedFilters.reference.toLowerCase())) return false;
    if (appliedFilters.account   && !String(dn.account_name ?? '').toLowerCase().includes(appliedFilters.account.toLowerCase())) return false;
    if (appliedFilters.amountFrom    && Number(dn.amount) < Number(appliedFilters.amountFrom)) return false;
    if (appliedFilters.amountTo      && Number(dn.amount) > Number(appliedFilters.amountTo))   return false;
    if (appliedFilters.unadjustedFrom && Number(dn.unadjusted_amount) < Number(appliedFilters.unadjustedFrom)) return false;
    if (appliedFilters.unadjustedTo   && Number(dn.unadjusted_amount) > Number(appliedFilters.unadjustedTo))   return false;
    if (!appliedFilters.showVoid && dn.status === 'cancelled') return false;
    return true;
  });
  ...
  async function handleApprove(dn: DebitNote) {
    if (!window.confirm(`Approve "${dn.number}"?`)) return;
    setActionId(dn.id);
    try { await approveDebitNote(dn.id); fetchNotes(); }
    catch (err: unknown) { alert(err instanceof Error ? err.message : 'Failed'); }
    finally { setActionId(null); }
  }

  async function handleCancel(dn: DebitNote) {
    const msg = dn.status === 'approved'
      ? `Cancel "${dn.number}"? This will reverse account balance changes.`
      : `Cancel "${dn.number}"?`;
    if (!window.confirm(msg)) return;
    setActionId(dn.id);
    try { await cancelDebitNote(dn.id); fetchNotes(); }
    catch (err: unknown) { alert(err instanceof Error ? err.message : 'Failed'); }
    finally { setActionId(null); }
  }

  async function handleDelete(dn: DebitNote) {
    if (!window.confirm(`Delete "${dn.number}"? This cannot be undone.`)) return;
    setActionId(dn.id);
    try { await deleteDebitNote(dn.id); fetchNotes(); }
    catch (err: unknown) { alert(err instanceof Error ? err.message : 'Failed'); }
    finally { setActionId(null); }
  }
    ...
    <div className="flex flex-col h-full bg-white">
      ...
      <div className="flex items-center justify-between px-6 pb-4">
        ...
        <div className="flex items-center gap-2">
          <button className="flex items-center gap-1.5 px-3 py-2 text-sm rounded bg-blue-600 text-white hover:bg-blue-700" onClick={() => printDebitNotes(visibleNotes)}>
            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z" />
            </svg>
            PRINT
          </button>
          <button className="flex items-center gap-1.5 px-3 py-2 text-sm rounded bg-blue-600 text-white hover:bg-blue-700" onClick={() => exportDebitNotesToExcel(visibleNotes)}>
            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
          ...
          <table className="w-full border-collapse">
            ...
            <tbody>
              {visibleNotes.length === 0 ? (
                ...
                visibleNotes.map((dn) => (
                  <tr key={dn.id} className="border-b border-gray-100 hover:bg-gray-50">
                    <td className="px-4 py-3 text-sm font-medium text-gray-800">{dn.number}</td>
                    <td className="px-4 py-3 text-sm text-gray-700">
                      {new Date(dn.date).toLocaleDateString('en-GB')}
                    </td>
                    <td className="px-4 py-3 text-sm text-gray-700">{dn.reference ?? ''}</td>
                    <td className="px-4 py-3 text-sm text-gray-800">{dn.contact_name}</td>
                    <td className="px-4 py-3 text-sm text-gray-800">{dn.account_name}</td>
                    <td className="px-4 py-3 text-sm text-right font-medium text-gray-800">{fmt(dn.amount)}</td>
                    <td className="px-4 py-3 text-sm text-right font-medium text-gray-800">{fmt(dn.unadjusted_amount)}</td>
                    <td className="px-4 py-3">
                      <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ${DN_STATUS_COLORS[dn.status]}`}>
                        {DN_STATUS_LABELS[dn.status]}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-right">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        {dn.status === 'approved' && (
                          <button onClick={() => handleCancel(dn)} disabled={actionId === dn.id}
                        ...
                        {dn.status === 'cancelled' && <span className="text-xs text-gray-400">—</span>}
                      </div>
                    </td>
                  </tr>
                ))
          ...
          <div className="relative bg-white rounded-lg border-2 border-green-400 shadow-2xl w-full max-w-lg mx-4">
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="flex items-center gap-3">
                <input type="checkbox" id="dnShowVoid"
                  className="w-4 h-4 rounded border-gray-300 text-blue-600"
                  checked={pendingFilters.showVoid}
                  onChange={(e) => setPendingFilters((f) => ({ ...f, showVoid: e.target.checked }))} />
                <label htmlFor="dnShowVoid" className="text-sm font-medium text-gray-700 cursor-pointer">Show Void</label>
              </div>
            </div>
