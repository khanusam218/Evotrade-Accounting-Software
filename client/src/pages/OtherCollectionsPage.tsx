...
import OtherCollectionForm from '../components/OtherCollectionForm';
import {
  approveOtherCollection, cancelOtherCollection,
  deleteOtherCollection, getOtherCollections,
} from '../api/otherCollections';
import { getBankAccountsLookup } from '../api/bankAccounts';
import type { OtherCollection } from '../types/otherTransaction';
...
const EMPTY_FILTERS: Filters = {
  number: '', dateFrom: '', dateTo: '', contact: '', reference: '',
  amountFrom: '', amountTo: '', bankAccountId: '', status: '', showVoid: false,
};

function printOtherCollections(rows: OtherCollection[]) {
  ...
  win.document.write(`<!DOCTYPE html><html><head><meta charset="utf-8"/><title>Other Collections</title>
...
<div class="hdr"><span class="co">Evotrade</span><span class="ti">Other Collections</span></div><hr/>
...
function exportOtherCollectionsToExcel(rows: OtherCollection[]) {
  ...
  const a=document.createElement('a');a.href=url;a.download='OtherCollections.csv';
  document.body.appendChild(a);a.click();document.body.removeChild(a);URL.revokeObjectURL(url);
}

export default function OtherCollectionsPage() {
  const [collections, setCollections] = useState<OtherCollection[]>([]);
  ...
  const [editEntry, setEditEntry] = useState<OtherCollection | null>(null);
  ...
  const fetchCollections = useCallback(async () => {
    ...
    try {
      const data = await getOtherCollections({
  ...
  const visibleCollections = collections.filter((oc) => {
    if (appliedFilters.contact      && !oc.contact_name.toLowerCase().includes(appliedFilters.contact.toLowerCase())) return false;
    if (appliedFilters.reference    && !String(oc.reference ?? '').toLowerCase().includes(appliedFilters.reference.toLowerCase())) return false;
    if (appliedFilters.amountFrom   && Number(oc.total_amount) < Number(appliedFilters.amountFrom)) return false;
    if (appliedFilters.amountTo     && Number(oc.total_amount) > Number(appliedFilters.amountTo))   return false;
    if (appliedFilters.bankAccountId && String(oc.bank_account_id) !== appliedFilters.bankAccountId) return false;
    if (!appliedFilters.showVoid    && oc.status === 'cancelled') return false;
    return true;
  });
  ...
  async function handleApprove(oc: OtherCollection) {
    if (!window.confirm(`Approve "${oc.number}"? This will update account balances.`)) return;
    setActionId(oc.id);
    try { await approveOtherCollection(oc.id); fetchCollections(); }
    catch (err: unknown) { alert(err instanceof Error ? err.message : 'Failed to approve'); }
    finally { setActionId(null); }
  }

  async function handleCancel(oc: OtherCollection) {
    const msg = oc.status === 'approved'
      ? `Cancel "${oc.number}"? This will reverse account balance changes.`
      : `Cancel "${oc.number}"?`;
    if (!window.confirm(msg)) return;
    setActionId(oc.id);
    try { await cancelOtherCollection(oc.id); fetchCollections(); }
    catch (err: unknown) { alert(err instanceof Error ? err.message : 'Failed to cancel'); }
    finally { setActionId(null); }
  }

  async function handleDelete(oc: OtherCollection) {
    if (!window.confirm(`Delete "${oc.number}"? This cannot be undone.`)) return;
    setActionId(oc.id);
    try { await deleteOtherCollection(oc.id); fetchCollections(); }
  ...
  return (
    <div className="flex flex-col h-full bg-gray-100">
      ...
      <div className="flex items-center justify-between bg-white border-b border-gray-200 px-6 py-3">
        <h1 className="text-lg font-bold text-gray-800">Other Collections</h1>
        <button
      ...
      <div className="flex items-center gap-2 bg-white border-b border-gray-200 px-6 py-2">
        ...
        <button className="flex items-center gap-1.5 text-sm font-semibold px-3 py-1.5 rounded bg-blue-600 text-white hover:bg-blue-700" onClick={() => printOtherCollections(visibleCollections)}>
          <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
        ...
        <button className="flex items-center gap-1.5 text-sm font-semibold px-3 py-1.5 rounded bg-blue-600 text-white hover:bg-blue-700" onClick={() => exportOtherCollectionsToExcel(visibleCollections)}>
          <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
            ...
            <table className="min-w-full text-sm">
              ...
              <tbody>
                {visibleCollections.length === 0 ? (
                  ...
                  visibleCollections.map((oc) => (
                    <tr key={oc.id} className="border-b border-gray-100 hover:bg-gray-50">
                      <td className="px-4 py-2.5 font-medium text-gray-800">{oc.number}</td>
                      <td className="px-4 py-2.5 text-gray-600 whitespace-nowrap">
                        {new Date(oc.date).toLocaleDateString()}
                      </td>
                      <td className="px-4 py-2.5 text-gray-600">{oc.reference ?? '—'}</td>
                      <td className="px-4 py-2.5 text-gray-800">{oc.contact_name}</td>
                      <td className="px-4 py-2.5 text-gray-600">
                        <div>{oc.bank_account_name}</div>
                        {oc.bank_name && <div className="text-xs text-gray-400">{oc.bank_name}</div>}
                      </td>
                      <td className="px-4 py-2.5 text-right font-mono text-gray-800">{fmt(oc.total_amount)}</td>
                      <td className="px-4 py-2.5">
                        <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ${STATUS_COLORS[oc.status] ?? ''}`}>
                      ...
                      <td className="px-4 py-2.5">
                        <div className="flex items-center justify-center gap-1">
                          {oc.status === 'draft' && (
                            <>
                              <button title="Edit" onClick={() => { setEditEntry(oc); setShowForm(true); }}
                              ...
                              <button title="Approve" onClick={() => handleApprove(oc)} disabled={actionId === oc.id}
                                className="p-1 rounded text-green-600 hover:bg-green-50 border border-green-200 disabled:opacity-50">
                                <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                                </svg>
                              </button>
                              <button title="Delete" onClick={() => handleDelete(oc)} disabled={actionId === oc.id}
                          ...
                          {oc.status === 'approved' && (
                            <button title="Cancel" onClick={() => handleCancel(oc)} disabled={actionId === oc.id}
                          ...
                          {oc.status === 'cancelled' && <span className="text-xs text-gray-400">—</span>}
                        </div>
                      </td>
                    </tr>
                  ))
          ...
          <div className="relative bg-white rounded-lg border-2 border-green-400 shadow-2xl w-full max-w-lg mx-4">
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="flex items-center gap-3">
                <input type="checkbox" id="ocShowVoid"
                  className="w-4 h-4 rounded border-gray-300 text-blue-600"
                  checked={pendingFilters.showVoid}
                  onChange={(e) => setPendingFilters((f) => ({ ...f, showVoid: e.target.checked }))} />
                <label htmlFor="ocShowVoid" className="text-sm font-medium text-gray-700 cursor-pointer">Show Void</label>
              </div>
            </div>
