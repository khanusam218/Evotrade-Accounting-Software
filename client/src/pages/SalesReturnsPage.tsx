...
import type { SalesReturn, SRStatus } from '../types/salesReturn';
import { SR_STATUS_COLORS, SR_STATUS_LABELS } from '../types/salesReturn';
import { getSalesReturns, approveSalesReturn, cancelSalesReturn, deleteSalesReturn } from '../api/salesReturns';
...
function printSalesReturns(rows: SalesReturn[]) {
  const win = window.open('', '_blank', 'width=1000,height=680');
  if (!win) return;
  win.document.write(`<!DOCTYPE html><html><head><title>Sale Returns</title>
...
<h2>Sale Returns</h2><table><thead><tr>
<th>Number</th><th>Date</th><th>Customer</th><th>Invoice</th>
<th>Net Amount</th><th>Status</th></tr></thead><tbody>
${rows.map(r => `<tr>
<td>${r.number || ''}</td><td>${r.date?.slice(0, 10) || ''}</td>
<td>${r.customer_name || ''}</td><td>${r.invoice_number || '—'}</td>
<td style="text-align:right">${Number(r.net_amount).toFixed(2)}</td>
<td>${SR_STATUS_LABELS[r.status] || r.status}</td></tr>`).join('')}
</tbody></table></body></html>`);
  win.document.close();
  setTimeout(() => win.print(), 400);
}

function exportSalesReturnsToExcel(rows: SalesReturn[]) {
  const header = ['Number', 'Date', 'Customer', 'Invoice', 'Net Amount', 'Unadjusted', 'Status'];
  const data = rows.map(r => [
    r.number, r.date?.slice(0, 10), r.customer_name, r.invoice_number || '',
    Number(r.net_amount).toFixed(2), Number(r.unadjusted_amount).toFixed(2),
    SR_STATUS_LABELS[r.status],
  ]);
  const csv = [header, ...data].map(r => r.map(v => `"${String(v ?? '').replace(/"/g, '""')}"`).join(',')).join('\n');
  const blob = new Blob([csv], { type: 'text/csv' });
  const url = URL.createObjectURL(blob);
  const a = document.createElement('a'); a.href = url; a.download = 'sale_returns.csv'; a.click();
...
export default function SalesReturnsPage() {
  ...
  async function load() {
    ...
    try { setRows(await getSalesReturns(p)); } finally { setLoading(false); }
  }
  ...
  if (appliedFilters.netAmountFrom) visible = visible.filter(r => Number(r.net_amount) >= Number(appliedFilters.netAmountFrom));
  if (appliedFilters.netAmountTo)   visible = visible.filter(r => Number(r.net_amount) <= Number(appliedFilters.netAmountTo));
  ...
  if (showForm) return (
        <SalesReturnForm ret={editing} onClose={() => setShowForm(false)} onSaved={() => { setShowForm(false); load(); }} />
  );
    ...
    <div className="p-6">
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-2">
          <button onClick={() => printSalesReturns(visible)}
            className="flex items-center gap-1.5 text-white bg-blue-600 hover:bg-blue-700 text-xs font-semibold px-3 py-1.5 rounded">
            <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z" />
            </svg>
            PRINT
          </button>
          <button onClick={() => exportSalesReturnsToExcel(visible)}
            className="flex items-center gap-1.5 text-white bg-blue-600 hover:bg-blue-700 text-xs font-semibold px-3 py-1.5 rounded">
            <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24">
          ...
          <thead>
            <tr className="bg-gray-50 border-b">
              {[
                ...
                { col: 'net_amount',        label: 'Net Amount' },
          ...
          <tbody>
            {loading ? (
              ...
              <tr key={r.id} className="border-b hover:bg-gray-50">
                ...
                <td className="px-3 py-1.5 text-xs text-right font-mono">{Number(r.net_amount).toFixed(2)}</td>
                <td className="px-3 py-1.5 text-xs text-right font-mono">{Number(r.unadjusted_amount).toFixed(2)}</td>
                <td className="px-3 py-1.5">
                  <span className={`inline-block px-2 py-0.5 rounded-full text-xs font-medium ${SR_STATUS_COLORS[r.status]}`}>
                    {SR_STATUS_LABELS[r.status]}
                  </span>
                </td>
                <td className="px-3 py-1.5">
                  <div className="flex items-center justify-center gap-1.5">
                    {r.status === 'draft' && (
                      <>
                        ...
                        {/* Approve */}
                        <button title="Approve" onClick={() => act(() => approveSalesReturn(r.id))} className="text-green-600 hover:text-green-800">
                          <svg viewBox="0 0 20 20" fill="currentColor" className="w-4 h-4">
                            <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
                          </svg>
                        </button>
                        {/* Delete */}
                        <button title="Delete" onClick={() => act(() => deleteSalesReturn(r.id))} className="text-red-500 hover:text-red-700">
                          <svg viewBox="0 0 20 20" fill="currentColor" className="w-4 h-4">
                    ...
                    {r.status === 'approved' && (
                      <button title="Cancel" onClick={() => act(() => cancelSalesReturn(r.id))} className="text-orange-500 hover:text-orange-700">
                        <svg viewBox="0 0 20 20" fill="currentColor" className="w-4 h-4">
                          <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" clipRule="evenodd" />
                        </svg>
                      </button>
                    )}
                    {(r.status === 'partially_adjusted' || r.status === 'fully_adjusted') && (
      ...
      {showFilters && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-white rounded-lg shadow-2xl w-full max-w-2xl border-2 border-green-500 mx-4">
            ...
            {/* Floating red × close button — top-right outside the modal */}
            ...
            {/* Fields — label on left, input on right */}
            <div className="px-8 py-6 space-y-4">
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="relative flex-1">
                  <select
                    ...
                    onChange={e => setPendingFilters(f => ({ ...f, customerId: e.target.value }))}>
                    <option value="">Type to search customer</option>
                    {customers.map(c => <option key={c.id} value={String(c.id)}>{c.print_name}</option>)}
                  </select>
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1 flex items-center gap-2">
                  ...
                  <input type="number" placeholder="From"
                    ...
                    value={pendingFilters.unadjustedFrom}
                    onChange={e => setPendingFilters(f => ({ ...f, unadjustedFrom: e.target.value }))} />
                  <span className="text-sm text-gray-600 shrink-0">To:</span>
                  <input type="number" placeholder="To"
                    className="flex-1 border border-gray-300 rounded px-2.5 py-1.5 text-sm focus:outline-none focus:ring-1 focus:ring-blue-400"
                    value={pendingFilters.unadjustedTo}
                    onChange={e => setPendingFilters(f => ({ ...f, unadjustedTo: e.target.value }))} />
                </div>
              </div>

              {/* Net Amount From/To */}
              <div className="flex items-center gap-4">
                <label className="w-36 shrink-0 text-sm font-medium text-gray-700">Net Amount</label>
                <div className="flex-1 flex items-center gap-2">
                  <span className="text-sm text-gray-600 shrink-0">From:</span>
                  <input type="number" placeholder="From"
                    className="flex-1 border border-gray-300 rounded px-2.5 py-1.5 text-sm focus:outline-none focus:ring-1 focus:ring-blue-400"
                    value={pendingFilters.netAmountFrom}
                    onChange={e => setPendingFilters(f => ({ ...f, netAmountFrom: e.target.value }))} />
                  <span className="text-sm text-gray-600 shrink-0">To:</span>
                  <input type="number" placeholder="To"
                    className="flex-1 border border-gray-300 rounded px-2.5 py-1.5 text-sm focus:outline-none focus:ring-1 focus:ring-blue-400"
                    value={pendingFilters.netAmountTo}
                    onChange={e => setPendingFilters(f => ({ ...f, netAmountTo: e.target.value }))} />
                </div>
              </div>
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="relative flex-1">
                  <select
                    ...
                    {(['draft', 'approved', 'partially_adjusted', 'fully_adjusted', 'cancelled'] as SRStatus[]).map(s => (
              ...
              {/* Product */}
              <div className="flex items-center gap-4">
                <label className="w-36 shrink-0 text-sm font-medium text-gray-700">Product</label>
                <div className="relative flex-1">
                  <select
                    className="w-full border border-gray-300 rounded px-3 py-1.5 text-sm focus:outline-none focus:ring-1 focus:ring-blue-400 bg-white appearance-none"
                    value={pendingFilters.productId}
                    onChange={e => setPendingFilters(f => ({ ...f, productId: e.target.value }))}>
